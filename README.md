...
3️⃣ 피어슨 상관계수와 히트맵으로 변수 간 상관관계를 파악했습니다.
분석 결과,
- 차량 연식이 최신일수록 가격이 높아지는 경향
- 주행거리가 짧을수록 가격이 높아지는 경향
- 엔진 크기가 클수록 가격이 높아지는 경향
...
프로젝트를 진행하며 도메인 이해와 데이터 처리 기준을 명확히 해야함을 깨달았습니다.
특히 어떤 기준으로 이상치를 정의하고 제거할 것인지 결정하는 과정이 가장 어려웠습니다.
단순히 모델 성능을 높이는 것보다 **도메인 맥락을 반영한 데이터 정제**가 가장 중요함을 알게되었습니다.
...
또한 의미 있는 데이터를 만들기 위해서는
**도메인을 이해하는 기획자와 기술을 담당하는 엔지니어 간 협업** 역시 중요한 스킬임을 알게되었습니다.
